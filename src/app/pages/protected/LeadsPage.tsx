...
const SOURCE_VALUES: readonly string[] = ['web', 'telegram', 'instagram']
...
type LeadConversationInfo = {
	sessionId: string
	lastMessageAt?: string
}

function readExpectedResponseAt(lead: Lead): string | undefined {
	const metadata = lead.metadata as Record<string, unknown> | undefined
	const value = metadata?.expected_response_at
	return typeof value === 'string' ? value : undefined
}

function readLastInboundAt(lead: Lead): string | undefined {
	const metadata = lead.metadata as Record<string, unknown> | undefined
	const value = metadata?.last_inbound_at
	return typeof value === 'string' ? value : undefined
}

function resolveSlaPresentation(
	lead: Lead,
	locale: string,
	t: (key: string, options?: Record<string, unknown>) => string,
): { label: string; tone: 'success' | 'warning' | 'danger' | 'info' } {
	const expected = readExpectedResponseAt(lead)
	if (!expected) {
		return {
			label: t('common.na'),
			tone: 'info',
	...
	const minutesLeft = Math.round((expectedMs - Date.now()) / 60000)
	const absMinutes = Math.abs(minutesLeft)
	const formatter = new Intl.RelativeTimeFormat(locale, { numeric: 'auto' })
	const relative = formatter.format(minutesLeft, 'minute')
...
function LeadsPage() {
	...
	const [conversationByLeadId, setConversationByLeadId] = useState<
		Map<string, LeadConversationInfo>
	>(() => new Map())
	...
	useEffect(() => {
		...
		async function loadConversations() {
			try {
				...
				const entries = new Map<string, LeadConversationInfo>()
				result.items.forEach((session: unknown) => {
					...
					const leadRecord =
						record.lead && typeof record.lead === 'object'
							...
							: null
					const leadId =
						leadRecord && typeof leadRecord.id === 'string'
							? leadRecord.id
							: null

					if (!leadId || typeof record.id !== 'string') {
						return
					}
				...
				setConversationByLeadId(entries)
			} catch {
				if (isActive) {
					setConversationByLeadId(new Map())
				}
			}
		}
		...
		const baseColumns: DataTableColumn<Lead>[] = [
				...
				render: lead => {
					...
					return (
						<div className='grid gap-0.5'>
							...
							<span className={tableSecondaryTextClassName}>
								{lead.ai_summary ?? t('leads.awaitingReply')}
							</span>
						</div>
					)
				},
			},
			{
				...
				render: lead => {
					...
					const lastInbound = readLastInboundAt(lead)
					...
					return (
						<div className='grid gap-0.5'>
							...
							<span className={tableSecondaryTextClassName}>
								{lastInbound
									? formatDate(lastInbound, locale, t('common.na'))
									: t('common.na')}
							</span>
						</div>
					)
			...
			},
			{
				...
				render: lead => {
					...
					const sessionId = linkedConversation?.sessionId
