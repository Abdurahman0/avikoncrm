...
const THEME_STORAGE_KEY = 'avikontex-theme'
...
function AppTopbar({
	...
	useEffect(() => {
		let isMounted = true
		const isChatRoute = location.pathname === routePaths.chats
		...
		async function loadChatSessionCount() {
			try {
				...
				setChatSessionCount(response.meta?.totalItems ?? response.total ?? 0)
			} catch {
				if (!isMounted) {
					return
				}
