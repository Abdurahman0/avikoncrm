...
function getStatusTone(
	...
	if (status === 'closed' || status === 'delivered' || status === 'signed' || status === 'completed') {
		return 'success'
	}
	if (status === 'canceled') {
		return 'danger'
	}
	if (status === 'confirmed' || status === 'packing' || status === 'shipped') {
		return 'accent'
	}
...
const CONTRACT_STAGE_IDS = ['new', 'confirmed', 'packing', 'shipped', 'delivered', 'closed'] as const
...
function getCurrentStageIndex(status: string): number {
	if (status === 'canceled' || status === 'closed' || status === 'signed' || status === 'completed') {
		return 5
	}
	if (status === 'delivered') {
		return 4
	}
	if (status === 'shipped' || status === 'sent' || status === 'payment_pending' || status === 'paid') {
		return 3
	}
	if (status === 'packing' || status === 'moderation' || status === 'contract_ready') {
		return 2
	}
	if (status === 'confirmed' || status === 'audit_pending' || status === 'audit_paid') {
		return 1
	}
	return 0
}
...
function getStageStatusLabel(
	...
	return getStatusLabel(stageId, isRu)
}
...
function ContractStatusTimeline({
	...
	const title = isRu ? '\u0425\u043e\u0434 \u0437\u0430\u043a\u0430\u0437\u0430' : 'Buyurtma jarayoni'
	...
	const canceledNote = isRu
		? '\u0417\u0430\u043a\u0430\u0437 \u043e\u0442\u043c\u0435\u043d\u0435\u043d. \u041f\u0440\u043e\u0446\u0435\u0441\u0441 \u043f\u043e \u0441\u0442\u0430\u0442\u0443\u0441\u0430\u043c \u043e\u0441\u0442\u0430\u043d\u043e\u0432\u043b\u0435\u043d.'
		: "Buyurtma bekor qilingan. Statuslar bo'yicha jarayon to'xtatilgan."
