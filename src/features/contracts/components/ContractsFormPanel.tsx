...
export function ContractsFormPanel({
	...
	const statusOptions = useMemo(
		() => [
			{ value: 'new', label: isRu ? 'Новый' : 'Yangi' },
			{ value: 'confirmed', label: isRu ? 'Подтвержден' : 'Tasdiqlandi' },
			{ value: 'packing', label: isRu ? 'Комплектуется' : "To'planyapti" },
			{ value: 'shipped', label: isRu ? 'Отправлен' : "Jo'natildi" },
			{ value: 'delivered', label: isRu ? 'Доставлен' : 'Yetkazildi' },
			{ value: 'closed', label: isRu ? 'Закрыт' : 'Yopildi' },
